import { Count, Routine } from '@/constants/Interfaces';
...
export const getRoutinesCount = async (db: SQLiteDatabase) : Promise<Count | undefined | null> => {
    try {
        return await db.getFirstAsync('SELECT COUNT(*) AS `count` FROM routines');
    } catch (err) {
        console.log("Error while loading routines count: ", err);
    }
}

export const getSingleRoutine = async (db: SQLiteDatabase, id: string) : Promise<Routine | null | undefined> => {
    try {
        return await db.getFirstAsync(`SELECT * FROM routines where routine_id = ${id}`);    
    } catch (err) {
        console.log(`Error while loading routine_id = ${id}: `, err);
    }
}
...
export const updateRoutine = async (db: SQLiteDatabase, newRoutine: {title: string, last_note: string, id: number}) => {
    console.log(newRoutine);
    try {
        const query = await db.prepareAsync(`UPDATE routines SET title = ?, last_note = ? WHERE routine_id = ?`);
        return await query.executeAsync(newRoutine.title, newRoutine.last_note, newRoutine.id);
    } catch (err) {
        console.log("Error while updating routine: ", err);
    }
}
